...
func NewHeaderChain(chainDb ethdb.Database, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	hashCache, _ := lru.New(numberCacheLimit)
...
func (hc *HeaderChain) CalcPastMedianTime(number uint64, parent *types.Header) *big.Int {
	...
	log.Debug(fmt.Sprintf("CalcPastMedianTime, number: %d", number))
	...
	for i, ancestor := range hc.GetBlockHeadersFromHash(iterNode.Hash(), medianTimeBlocks) {
		log.Debug(fmt.Sprintln("CalcPastMedianTime, timestamps: %v", timestamps))
	...
	log.Debug(fmt.Sprintf("consensus -> calcPastMedianTime - starting: %d, ending: %d, parent != nil: %t", number, limit, parent != nil))

	for i := number; i >= limit; i-- {
		if parent != nil && i == number {
			log.Debug(fmt.Sprintf("consensus -> calcPastMedianTime - parent: %d", parent.Number))
			timestamps[numNodes] = parent.Time
		} else {
			log.Debug(fmt.Sprintf("consensus -> calcPastMedianTime - GetHeaderByNumber: %d", i))
			log.Debug(fmt.Sprintf("numnodes: %d, timestamps: %v", numNodes, timestamps))
...
func (hc *HeaderChain) GetHeaderByNumber(number uint64) *types.Header {
	...
	if cache, ok := hc.hashCache.Get(number); ok {
		log.Debug(fmt.Sprintf("GetHeaderByNumber, cached true, %d", number))
		return hc.GetHeader(cache.(common.Hash), number)
	}

	hash := GetCanonicalHash(hc.chainDb, number)
	if hash == (common.Hash{}) {
		log.Debug(fmt.Sprintf("GetHeaderByNumber, cached false, hash nil"))
		return nil
	}
	log.Debug(fmt.Sprintf("GetHeaderByNumber, cached false, db true"))
