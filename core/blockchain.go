...
func (self *BlockChain) CalcPastMedianTime(number uint64, parent *types.Header) *big.Int {
	return self.hc.CalcPastMedianTime(number, parent)
}
...
func (self *BlockChain) GetBlockHeadersFromHash(hash common.Hash, max uint64) []*types.Header {
	return self.hc.GetBlockHeadersFromHash(hash, max)
}
